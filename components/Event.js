...
const Event = ({ route, navigation }) => {
			...
			try {
				...
				if (tab === "Cluster") {
					if (event.participants.length > 2) {
						const getUsersPromises = event.participants.map(participantId =>
							supabase
								.from("users")
								.select()
								.eq("user_id", participantId)
			...
			const response = await axios.post(
				"https://api.openai.com/v1/chat/completions",
				{
					...
					messages: [
						{
							role: "system",
							content: "You are a helpful assistant that identifies users similar to a given user based off provided data."
						},
						{
							role: "user",
	...
	const handleJoin = async () => {
		if (!response) {
			Alert.alert("Uhoh", "A response is needed to join the event!");
		}
		...
		const { error } = await supabase
			...
			.update({
				...
				participant_responses: event.participant_responses
			})
			.eq("event_id", eventId);
	...
	const renderUsers = () => {
		if (tab === "All Users") {
			if (users.length !== 0 || users) {
				...
				return <Text style={styles.ended}>No other Users!</Text>;
			}
		}
		if (tab === "Cluster") {
			if (users.length !== 0 || users) {
				...
				return <Text style={styles.cluster}>Not enough Eventgoers to form a cluster!</Text>;
			}
		}
	};
	...
	const renderBottom = () => {
		...
		if (
			(currentDateTime <= event.end_time && !event.participants.includes(userId)) ||
			(currentDateTime < event.start_time && event.participants.includes(userId))
		) {
			return (
				<View>
						...
						{loading ? (
							...
							<Pressable style={styles.button} onPress={handleJoin}>
								<Text style={styles.buttonText}>
									{event.participants.includes(userId) ? "Save Response" : "Join Event"}
								</Text>
							</Pressable>
						)}
					</View>
	...
	return (
		<LinearGradient colors={["#A8D0F5", "#D0B4F4"]} style={styles.eventContainer}>
			...
			<Text style={styles.otherText}>{`From ${formatDate(event.start_time)} To ${formatDate(
				event.end_time
			)}`}</Text>
			<Text style={styles.otherText}>{`${
				event.participants ? event.participants.length : 0
			} person(s) ${currentDateTime > event.end_time ? "went" : "going"}`}</Text>
			{renderBottom()}
